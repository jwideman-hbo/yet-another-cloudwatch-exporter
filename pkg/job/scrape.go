		...
		for _, role := range discoveryJob.Roles {
			for _, region := range discoveryJob.Regions {
				...
				go func(discoveryJob model.DiscoveryJob, region string, role model.Role) {
					...
					var gmdClient getmetricdata.Client = cloudwatchClient
					...
					resources, metrics := runDiscoveryJob(ctx, jobLogger, discoveryJob, region, factory.GetTaggingClient(region, role, taggingAPIConcurrency), cloudwatchClient, gmdProcessor, accountID, jobsCfg.OwnerMetricExclusions)
		...
		for _, role := range customNamespaceJob.Roles {
			for _, region := range customNamespaceJob.Regions {
				...
				go func(customNamespaceJob model.CustomNamespaceJob, region string, role model.Role) {
					...
					metrics := runCustomNamespaceJob(ctx, jobLogger, customNamespaceJob, cloudwatchClient, gmdProcessor, ownerTagClient, accountID, region, jobsCfg.OwnerMetricExclusions)
					metricResult := model.CloudwatchMetricResult{
						Context: &model.ScrapeContext{
							Region:     region,
							AccountID:  accountID,
