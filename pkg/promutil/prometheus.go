...
var (
	CloudwatchGetMetricDataOwnerQueryObjectsCounter = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Logical GetMetricData query objects submitted by resource owner; excludes owner metric exclusions, internal SDK retries, and pagination.",
	...
	CloudwatchGetMetricDataOwnerPreExclusionEstimatedUnitsCounter = prometheus.NewCounterVec(prometheus.CounterOpts{
		Name: "yace_cloudwatch_getmetricdata_owner_pre_exclusion_estimated_metric_requests_total",
		Help: "Estimated GetMetricData metric-request units before owner metric exclusions, using the configured query batch size. Subtract submitted estimated units to estimate avoided units.",
	...
	CloudwatchGetMetricDataOwnerExcludedQueryObjectsCounter = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Direct GetMetricData query objects excluded before submission by an owner metric exclusion rule. This is not a billing-unit counter.",
	}, []string{"target_account_id", "target_region", "cloudwatch_namespace", "cloudwatch_metric_name", "owner_business_service", "owner_service", "owner_component"})
